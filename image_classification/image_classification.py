...
def main():
    ...
    parser.add_argument('--optimizer', type=str, default='lamb', choices=['lamb', 'adam','lars', 'sgd'],
    ...
    writer = SummaryWriter(comment="_cv_%s_%s_%s" % (args.optimizer, args.batch_size, args.lr))
    ...
    if args.optimizer == 'lamb':
        ...
        base_optimizer = torch.optim.SGD(model.parameters(), lr=args.lr, momentum=0.9)
        optimizer = LARS(optimizer=base_optimizer, eps=1e-8, trust_coef=0.001, writer=writer)
    elif args.optimizer == 'sgd':
        optimizer = SGD(model.parameters(), lr=args.lr, momentum=0.9,
                        weight_decay=args.wd, writer=writer)
    else:
        # use adam optimizer
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        pickle.dump(metrics, open(os.path.join("cv_results", '%s_%s_metrics.p' % (args.optimizer, args.batch_size)), 'wb'))
